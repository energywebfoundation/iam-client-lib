...
describe('Staking service tests', () => {
  ...
  beforeAll(async () => {
    ...
    ({ connectToCacheServer, signerService } = await initWithPrivateKeySigner(
  ...
  it('should be able to get all services', async () => {
    ...
    expect(
      (await stakingService.allServices()).map((s) => ({ org: s.org, provider: s.provider }))
    ).toStrictEqual([
