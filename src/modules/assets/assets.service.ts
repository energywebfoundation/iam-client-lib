...
export class AssetsService {
    ...
    private _did: string;
    private _assetManager: IdentityManager;
    ...
    async init() {
        ...
        this._assetManager = new IdentityManager__factory().attach(assetManagerAddress);
    }

    async registerAsset(): Promise<string> {
        const data = this._assetManager.interface.encodeFunctionData("createIdentity", [this._owner]);
        const receipt = await this._signerService.send({ to: this._assetManager.address, data });
        const event = receipt.logs
            .map((l) => this._assetManager.interface.parseLog(l))
            .find(
                (log) =>
                    log.name === this._assetManager.interface.events["IdentityCreated(address,address,uint256)"].name,
    ...
    private offerAssetTx({
        ...
        const asset = new OfferableIdentity__factory().attach(assetContractAddress);
        return {
