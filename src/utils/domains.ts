...
} from './domains-governance-build.js';
...
export const transferEwcEngie = async (privateKey: string) => {
  const engie = 'engie.auth.ewc';
  const signer = new Wallet(privateKey);
  const { ensRegistry } = await initDomains(signer, EWC_CHAIN_ID);
