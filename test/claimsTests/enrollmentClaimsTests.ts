...
export function enrollmentClaimsTests() {
  ...
  const roles: Record<string, IRoleDefinition> = {
    [`${roleName1}.${root}`]:
      { ...baseRoleDef, roleName: roleName1 },
    [`${roleName2}.${root}`]:
      { ...baseRoleDef, roleName: roleName2, issuer: { issuerType: "ROLE", roleName: `${roleName1}.${root}` } },
    [`${roleName3}.${root}`]:
      { ...baseRoleDef, roleName: roleName3, enrolmentPreconditions: [{ type: PreconditionType.Role, conditions: [`${roleName1}.${root}`] }] }
  };
  ...
  async function enrolAndIssue(
    requesterIam: IAM,
    issuerIam: IAM,
    { subjectDID, claimType, registrationTypes = [RegistrationTypes.OnChain] }:
  ...
  test("enrollment by issuer of type DID", async () => {
    await enrolAndIssue(
      userIam,
      staticIssuerIam,
      { subjectDID: userDID, claimType: `${roleName1}.${root}` }
    );
  ...
  test("asset enrollment by issuer of type DID", async () => {
    ...
    await enrolAndIssue(
      userIam, staticIssuerIam,
      { subjectDID: assetDID, claimType: `${roleName1}.${root}` }
    );
  ...
  test("enrollment by issuer of type ROLE", async () => {
    ...
    await enrolAndIssue(
      dynamicIssuerIam, staticIssuerIam,
      { subjectDID: dynamicIssuerDID, claimType: `${roleName1}.${root}` }
    );

    expect(await claimManager.hasRole(addressOf(dynamicIssuerDID), namehash(`${roleName1}.${root}`), version));

    enrolAndIssue(
      userIam, dynamicIssuerIam,
      { subjectDID: userDID, claimType: `${roleName2}.${root}` }
    );
  ...
  test("should enrol when prerequisites are met", async () => {
    ...
    await enrolAndIssue(
      userIam, staticIssuerIam,
      { subjectDID: userDID, claimType: `${roleName3}.${root}` }
    );
